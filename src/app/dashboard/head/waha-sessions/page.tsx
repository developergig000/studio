...
export default function WahaSessionsPage() {
    ...
    try {
      const res = await fetch('/api/integrations/waha/action', {
        ...
        body: JSON.stringify({
          action,
          userId,
          sessionName, // Pass the configured session name, can be undefined
        }),
      });
      ...
      if (action === 'start') {
        ...
        // The session name might be newly generated by the API or the one we passed.
  ...
  const displayUsers = React.useMemo((): MergedUser[] => {
    return salesUsers.map(user => {
      // Use the configured session name from firestore as the primary identifier
      const sessionName = user.wahaSessionName;
      const liveSession = sessionName ? liveSessions.find(s => s.name === sessionName) : undefined;
